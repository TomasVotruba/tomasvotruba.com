...
perex: |
    ...
    <br><br>
    In this series, you can learn about my experience, tricks, tips, what works and what fucked me up. So you **save some frustration, where is not needed, discover hidden shortcuts and cool tools you never saw before**.
    <br><br>
...
In PSR-4 migration we have clear goal:

- PSR-4: in 1 file, there is 1 class/trait/interface
- the class/trait/interface has unique fully qualified name, that refects file location
- nothing else exists

### Start has Many Ugly Forms

- **in 1 file, there is dozen classes** - very popular for exceptions or test fixtures
- there are classes, that have **identical name** and custom class loader gives one or the other prefference (e.g. Magento and Drupal use this in some version)
...
Always go for simple target first. Don't a be hero. A hero, falls from the sky after huge battle over Atlantic, forgot  to charge his smartphone... and dies alone.

Be proffesional, close early. Are there 3 files with 20 classes in them?
...
But all those 9 steps are now 10 % less complex.

<blockquote class="blockquote text-center mt-3 mb-3">
    "Even if you die, the code your merge is done."
    <footer class="blockquote-footer text-right">Bus Boy Scout Factor</footer>
</blockquote>

I love this coding principle. Why? Because it takes minimalism and productivity to practical worlds. It narrows our focus, so any developer become 10x programmer effortlessly.
...
[They're hiriging](https://www.startupjobs.cz/en/startup/scrumworks-s-r-o) and this is not paid promo. We're far enough with
the migration, so I'm confident it would be fun to work with such codebase.
...
It doesn't need projects' autoloader, so it can be installed outside the project, e.g. in `/var/www/tools`, while your project is in `/var/www/old_project`.
...
It would be great to have list of all such multi-class files, right?
...
Now we know, how big problem we're dealing with.

- Is that 3 files with 20 classes? **Separate them manually**.
...
Send pull request, make sure they're autoload by your project's autoloader and tests are passing. Merge it and you're done.

## 2. Check class short name vs File name
...
You will get list of files that doesn't match. Use PHPStorm refactoring to change the class name everywhere:
...
In PSR-4, any non-root directory must start with first big letter. Root is e.g. `/app`, `/src`.
...
This will generate such paths for us, based on existing namespaces and file locations.
It is possible there will be over 10 or even 50 of those. Don't worry about it now.
...
This is my favorite part. Here we basically move all directories **to use as little namespace root as possible**.

It might be little bit unclear, but give it a time and it will fit in. Lets look at the example:
...
Just register them in `rector.yaml`:
...
And manually add desired namespace to your `composer.json`:
...
This is one of the biggest changes in your application, so be sure to check it properly. Not all cases are covered by Rector yet.
...
Then we added Few manual tweaks here and there and we were PSR-4 compliant with ~7 lines in in PSR-4 in `composer.json`.

<br>

**Have you found a case that is not covered or better way to this**? Let me know in the comments.
