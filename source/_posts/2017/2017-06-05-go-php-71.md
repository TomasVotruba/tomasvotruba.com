...
perex: '''
    ...
    <br><br>
    Why skip PHP 7.0 and go directly to PHP 7.1? When it's the best time to join?
...
- **Frameworks are migrating**. And let's be honest - huge frameworks are flag ships of evolution. Join them and you won't be let behind. 
...
As in every new version of PHP, **most of us still <strike>depends</strike> waits on hosts to catch up**. The situation was the same with PHP 5. New software version was already there, but hosts took their time. If there would be some way to move this.
